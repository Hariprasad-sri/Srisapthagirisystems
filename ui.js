...
export function initIcons() {
    ...
    // Apply role-based visibility for dynamically rendered elements
    const role = state.currentUser || sessionStorage.getItem('ss_user');
    if (role === 'admin') {
...
export function getPipeType(product) {
...
export function getPipeSize(product) {
...
function renderPipeDashboard() {
    ...
    if (tabs.length === 0) {
        ...
        document.getElementById('pipe-dashboard-body').innerHTML = `<tr><td colspan="${2 + columns.length}" style="padding:24px; text-align:center; color:var(--text-muted);">No pipe records found.</td></tr>`;
        return;
    }
    ...
    if (tabsContainer) {
        tabsContainer.innerHTML = tabs.map(tab => {
            const escTab = tab.replace(/'/g, "\\'").replace(/"/g, "&quot;");
            return `
                <button class="pipe-tab ${tab === currentPipeTab ? 'active' : ''}" onclick="window.setPipeTab('${escTab}')">${tab}</button>
            `;
        }).join('');
    }
    ...
    const rows = sizes.map(size => {
        ...
        const escSize = size.toString().replace(/'/g, "\\'").replace(/"/g, "&quot;");
        const escPipeType = pipeType.toString().replace(/'/g, "\\'").replace(/"/g, "&quot;");
            ...
            <tr style="border-bottom:1px solid #e2e8f0;">
                <td style="padding:18px 16px; color:#0f172a; font-weight:700; font-size:15px;">
                    <div style="display:flex; align-items:center;">
                        ${size} <i data-lucide="edit-2" class="cursor-pointer admin-only" style="margin-left: 10px; color: #0f172a; opacity:0.8;" size="14" onclick="window.editPipeSize('${escSize}', '${escPipeType}')" title="Edit Size"></i>
                    </div>
                </td>
                ${values.join('')}
                <td style="padding:18px 16px; text-align:center;">
                    ${firstProd ? `<button class="glass-btn admin-only" style="padding:4px;color:var(--danger);background:rgba(239,68,68,0.05);margin:0 auto;" title="Delete Row" onclick="window.deletePipeSizeRow('${escSize}', '${escPipeType}')">
                        <i data-lucide="trash-2" size="14"></i>
                    </button>` : ''}
                </td>
            </tr>
    ...
    if (body) {
        body.innerHTML = rows.join('') || `<tr><td colspan="${2 + columns.length}" style="padding:24px; text-align:center; color:var(--text-muted);">No pipe items exist for ${currentPipeTab}.</td></tr>`;
    }
    initIcons();
}
...
export function renderFittingMatrix() {
    ...
    if (tabsContainer) {
        tabsContainer.innerHTML = tabs.map(tab => {
            const escTab = tab.replace(/'/g, "\\'").replace(/"/g, "&quot;");
            return `
                <button class="pipe-tab ${tab === currentFittingTab ? 'active' : ''}" onclick="window.setFittingTab('${escTab}')">
                    ${tab}
                </button>
            `;
        }).join('');
    ...
    if (thead) {
        const escTab = currentFittingTab.replace(/'/g, "\\'").replace(/"/g, "&quot;");
        thead.innerHTML = `
            <tr>
                ...
                <th class="admin-only" style="cursor:pointer; color:var(--primary); padding:18px 16px; font-size:13px; font-weight:700; text-align:center;" onclick="window.addFittingSize('${escTab}')">
                    + Add Size
                </th>
            </tr>
        `;
            ...
            tbody.innerHTML += `
                <tr class="admin-only">
                    <td colspan="${sizes.length + 2}" style="padding:16px;">
                        <button class="glass-btn" style="color:var(--primary); font-weight:600;" onclick="window.addFittingItem('${escTab}')">

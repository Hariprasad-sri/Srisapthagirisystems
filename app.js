import { renderAll, renderInventory, renderRequests, initIcons, getPipeType, getPipeSize } from './ui.js';
...
// Expose to global scope so the modal's inline onclick can call it (app.js is a module)
...
function addNewPipeColumn() {
    const input = document.getElementById('new-pipe-column-input');
    const columnName = input.value.trim().toUpperCase();
    ...
    const list = document.getElementById('pipe-columns-list');
    const existing = Array.from(list.querySelectorAll('input[name="pipe-column"]')).map(el => el.value);
    ...
    const label = document.createElement('label');
    label.innerHTML = `<input type="checkbox" name="pipe-column" value="${columnName}" checked> ${columnName}`;
...
window.addNewPipeColumn = addNewPipeColumn;
...
async function deletePipeColumnFromTable(columnName) {
    if (!confirm(`Delete column "${columnName}" from the pipe dashboard?`)) return;
    try {
        // Remove from state
        ...
        // Save updated columns list to backend
        await savePipeColumns(state.pipeColumns);
        // Re-render the dashboard
        renderPipeDashboard();
    } catch (err) {
        console.error('Could not delete column:', err);
        alert('Could not delete column.');
    }
}

window.deletePipeColumnFromTable = deletePipeColumnFromTable;
...
function setupEventListeners() {
    ...
    window.editPipeSize = async (oldSize, subCategory) => {
        ...
        const productsToUpdate = state.products.filter(p => {
            const pSize = (p.specs?.size || p.size || p.model || '—').toString().trim();
            const pSubCat = (p.subCategory || 'General').trim();
            return p.category === 'supreme' && pSize === oldSize.trim() && pSubCat === subCategory.trim();
        });
    ...
    window.deletePipeSizeRow = async (size, subCategory) => {
        ...
        const productsToDelete = state.products.filter(p => {
            const pSize = (p.specs?.size || p.size || p.model || '—').toString().trim();
            const pSubCat = (p.subCategory || 'General').trim();
            return p.category === 'supreme' && pSize === size.trim() && pSubCat === subCategory.trim();
        });
    ...
    window.deleteFittingSize = async (size, tab) => {
        ...
        const productsToDelete = state.products.filter(p => {
            const pSubCat = (p.subCategory || 'PVC FITTINGS').trim();
            const pSize = (p.specs?.size || p.size || p.model || '—').toString().trim();
            return p.category === 'fitting' && pSize === size.trim() && pSubCat === tab.trim();
        });
        ...
        try {
            ...
            console.error(err);
            alert('Error deleting size column');
        }
    };
    ...
    window.deleteFittingItem = async (itemName, tab) => {
        ...
        const productsToDelete = state.products.filter(p => {
            if (p.category !== 'fitting') return false;
            const pSubCat = (p.subCategory || 'PVC FITTINGS').trim();
            if (pSubCat !== tab.trim()) return false;
            
            let n = p.name || '';
            const tabStr = pSubCat.replace(' FITTINGS', '').replace(' FITTING', '');
            if (n.toUpperCase().startsWith(tabStr.toUpperCase())) n = n.substring(tabStr.length).trim();
            if (n.toUpperCase().startsWith('FITTINGS')) n = n.substring(8).trim();
            if (n.toUpperCase().startsWith('FITTING')) n = n.substring(7).trim();
            const sz = (p.specs?.size || p.size || '-').toString().trim();
            if (sz !== '-' && n.endsWith(sz)) n = n.substring(0, n.length - sz.length).trim();
            return n.toUpperCase() === itemName.trim().toUpperCase();
        });
...
function renderLocationList() {
    ...
    list.innerHTML = state.locations.map(loc => {
        const escLoc = loc.name.replace(/'/g, "\\'").replace(/"/g, "&quot;");
        return `
        <div class="flex justify-between items-center glass-row" style="padding:10px; border:1px solid var(--border-light); border-radius:12px; background:rgba(255,255,255,0.4); margin-bottom:4px;">
            ...
            <button class="glass-btn" style="padding:6px; color:var(--danger); background:rgba(239,68,68,0.05);" onclick="deleteLocation('${escLoc}')">
